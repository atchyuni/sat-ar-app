...
def process_avatar():
    ...
    try:
        print(f"--- DEBUG MODE: DOWNLOADING ONLY ---")
        ...
        new_avatar_url = request.host_url.rstrip('/') + f"/files/{original_filename}"
        
        print(f"--- DEBUG MODE: RETURNING ORIGINAL URL: {new_avatar_url} ---")
        return jsonify({"newUrl": new_avatar_url})

    except Exception as e:
        print(f"download error: {e}")
        return jsonify({"error": "pipeline failed during download"}), 500
